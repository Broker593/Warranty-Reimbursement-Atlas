(() => {
  ...
  function renderCoverage() {
    ...
    if(ui.view==='labor')$('coverage').innerHTML='<strong>Coverage research v3</strong><span>50 states · 200 entries</span><span>40 official-statute records · 5 enacted-law records · 5 code reproductions</span><span>Research dated September 25, 2026</span>';
  ...
  function ruleExplanation(s,id) {
    ...
    const body=id==='accuracy_only'?s.process?.challenge:id==='automatic_rate'?s.process?.rateApproval:r.group==='Parts'?s.parts:r.group==='Hourly rate'?s.labor:r.group==='Paid hours'?(effectiveTime(s)):s.sample;
  ...
  function renderLabor() {
    ...
    $('programScope').hidden=!type.programScope;$('programScope').textContent=type.programScope||'';
    $('laborProgress').textContent='50 states researched · 200 coverage entries populated · Claude v3 research dated September 25, 2026';
    $('coverageTally').innerHTML='<caption>Coverage counts · all 50 states · as of '+esc(ui.asOf)+'</caption><thead><tr><th scope="col">Coverage</th>'+labor.statuses.map(s=>'<th scope="col">'+s.name+'</th>').join('')+'</tr></thead><tbody>'+labor.tallies(ui.asOf).map(t=>'<tr class="'+(t.coverage===ui.coverage?'current-coverage':'')+'"><th scope="row">'+esc(t.name)+(['manufacturer_contract','cpo'].includes(t.coverage)?'<small class="tally-scope">Issuer / obligor conditions apply · see program scope below</small>':'')+'</th>'+labor.statuses.map(s=>'<td><button data-coverage-count="'+t.coverage+'" data-count-status="'+s.id+'" aria-label="Show '+t.counts[s.id]+' '+s.name+' states for '+esc(t.name)+'">'+t.counts[s.id]+'</button></td>').join('')+'</tr>').join('')+'</tbody>';
    $('laborDateNotice').textContent='Counts and state entries reflect documented rules as of '+ui.asOf+'. '+(ui.asOf<'2026-10-01'?'Rhode Island’s October 1 changes are UPCOMING: factory hours count as Statute silent on time and CPO as Not addressed. Select October 1, 2026 to see the v3 post-effective-date totals.':'Rhode Island’s October 1, 2026 changes are EFFECTIVE: factory hours count as Actual technician time and CPO as Required.');
    $('hoursHeading').textContent='Paid-hours counts · '+type.name+' · as of '+ui.asOf;
    ...
    $('laborCountNote').textContent='One primary paid-hours bucket per state and coverage type. Counts cover all 50 states at the selected date, independent of search or comparison filters. Mississippi’s qualified-technician benchmark is included in Factory time. NC and PA service-contract entries without established hours are included in Statute silent on time. Wisconsin stays Negotiated/other because its time adjustment is in the hourly-rate formula; its paid hours remain OEM time.';
  ...
  function openLaborState(s) {
    ...
    $('stateContent').innerHTML='<div class="state-summary">'+statusBadge(r)+'<p>'+esc(r.applicabilityNote)+'</p>'+dateBadge(r)+(r.programScope?'<p class="program-scope">'+esc(r.programScope)+'</p>':'')+'<p><strong>Confidence: '+esc(r.confidence)+'</strong> · '+esc(r.sourceName)+'<br>Claude v3 research: September 25, 2026. Imported September 25, 2026.</p></div><div class="state-sections"><section class="state-section"><div class="section-label">Hourly dollar rate</div><p>'+esc(r.hourlyRate)+'</p>'+(r.contextOnly?'<details><summary>State’s factory-warranty rate method, for context only</summary><p>'+esc(r.stateHourlyRate)+'</p></details>':'')+'</section><section class="state-section"><div class="section-label">Paid hours</div><h3>'+esc(r.paidHours)+'</h3><p>'+esc(r.hoursNote)+'</p></section><section class="state-section"><div class="section-label">'+esc(r.evidenceLabel)+'</div>'+ (r.currentEvidence?'<aside class="current-evidence"><strong>Text in force on the selected date</strong><blockquote class="statute-quote">'+esc(r.currentEvidence.quote)+'</blockquote><p>'+esc(r.currentEvidence.pinpoint)+' · <a href="'+esc(r.currentEvidence.url)+'" target="_blank" rel="noopener">Read prior text ↗</a></p><p><strong>Upcoming enacted quotation:</strong></p></aside>':'')+(r.scopeEvidence?'<aside class="current-evidence"><strong>Scope evidence · claim approval/payment timing only</strong><blockquote class="statute-quote">'+esc(r.scopeEvidence.quote)+'</blockquote><p>'+esc(r.scopeEvidence.pinpoint)+'</p><p><strong>Warranty-rate limitation quoted in supplied record:</strong></p></aside>':'')+'<blockquote class="statute-quote">'+esc(r.quote)+'</blockquote><p><strong>Pinpoint: '+esc(r.pinpoint)+'</strong></p><p>Primary citation: '+esc(r.primaryCite)+'</p><p>Definitions checked: '+esc(r.definitionCite)+'</p></section><section class="state-section"><div class="section-label">State notes and qualifications</div><p>'+esc(r.notes)+'</p></section><section class="state-section"><div class="section-label">Effective dates and legislative history</div><p>'+esc(r.effectiveNotes)+'</p></section><section class="state-section"><div class="section-label">Sources and provenance</div><p>Research source type: '+esc(r.sourceName)+'. Source classification and confidence are from the supplied v3 research. This import is not a claim that every source was independently rechecked.</p><p>'+esc(r.sourceNote)+'</p><div class="source-links">'+links+'</div><p><a href="research/labor-by-coverage-v3.json" download>Download supplied JSON</a> · <a href="https://github.com/Broker593/Warranty-Reimbursement-Atlas/blob/main/docs/research/labor-by-coverage-patch-v3.md" target="_blank" rel="noopener">Read supplied research patch ↗</a></p></section></div><div class="detail-actions"><button class="quiet" data-detail-select="'+s.abbr+'">'+(ui.selected.has(s.abbr)?'Remove from comparison':'Add to comparison')+'</button><button class="primary" id="shareState">Share this state ↗</button></div>';
  ...
  $('asOf').addEventListener('change',e=>{if(!e.target.value || !e.target.validity.valid)return;ui.asOf=e.target.value;render();if(ui.state)openState(ui.state,ui.detailRule);});
